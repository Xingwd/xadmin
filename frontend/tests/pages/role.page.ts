...
export class RolePage {
    ...
    constructor(page: Page) {
        ...
        this.infoButton = page.locator('.table-row-info')
    }
    ...
    async addRole(name: string) {
        ...
        await form.locator('.el-form-item').nth(0).locator('input').fill(name)

        await this.saveButton.click()
    }

    async editFirstRole(name: string) {
        ...
        const nameInput = form.locator('.el-form-item').nth(0).locator('input')
    ...
    async viewInfo() {
        await this.tableRows.first().locator('.table-row-info').click()
        await expect(this.dialog).toBeVisible()
